...
<body class="bg-gray-100 font-sans text-gray-800 antialiased loading">
    ...
    <div id="dashboard-main-content" class="dashboard-content-fade-in">
        ...
        <main class="max-w-7xl mx-auto px-4 py-10 space-y-12">
            ...
            <section class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-7">
                <div class="bg-white p-6 rounded-xl shadow-lg border border-gray-200 hover:shadow-xl transition duration-300 ease-in-out cursor-pointer transform hover:-translate-y-1 card-products" onclick="window.location.href='products.php'">
                ...
                <div onclick="window.location.href='products.php?lowstock=1'" class="bg-white p-6 rounded-xl shadow-lg border border-gray-200 hover:shadow-xl transition duration-300 ease-in-out cursor-pointer transform hover:-translate-y-1 card-low-stock">
                    <div class="flex items-center justify-between mb-4">
                ...
                <div class="bg-white p-6 rounded-xl shadow-lg border border-gray-200 hover:shadow-xl transition duration-300 ease-in-out cursor-pointer transform hover:-translate-y-1 card-total-sales" onclick="window.location.href='billing_history.php'">
                    <div class="flex items-center justify-between mb-4">
                    ...
                    <div class="text-5xl font-bold text-gray-900 card-value">₹<?= number_format($total_sales, 2) ?></div>
                </div>

                <div class="bg-white p-6 rounded-xl shadow-lg border border-gray-200 hover:shadow-xl transition duration-300 ease-in-out cursor-pointer transform hover:-translate-y-1 card-sales-today" onclick="redirectToTodaySales()">
                    <div class="flex items-center justify-between mb-4">
                    ...
                    <div class="text-5xl font-bold text-gray-900 card-value">₹<?= number_format($sales_today, 2) ?></div>
                </div>
        ...
        fetch('get_dashboard_stats.php')
            ...
            .then(data => {
                ...
                if (data.success) {
                    ...
                    document.querySelector('.card-products .card-value').textContent = data.total_products;
                    
                    // Update low stock products
                    document.querySelector('.card-low-stock .card-value').textContent = data.low_stock_products;
                    
                    // Update total sales
                    document.querySelector('.card-total-sales .card-value').textContent = `₹${data.total_sales}`;
                    
                    // Update sales today
                    document.querySelector('.card-sales-today .card-value').textContent = `₹${data.sales_today}`;
                } else {
    ...
    // Set up a timer to update the stats every 10 seconds
    setInterval(updateDashboardStats, 100); // 10000 milliseconds = 10 seconds
